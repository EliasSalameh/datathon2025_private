...
def RandomForestClassifier_train(X_train, y_train, save_model=True, k=20):
    ...
    if save_model:
        joblib.dump(rf_model, 'rf_model_topK_full_filtered.pkl')

    return rf_model

def RandomForestClassifier_predict(rf_model, X_val, y_val=None):
...
def CatBoost_Train(X_train, y_train, k = 20, save_model=True):
    ...
    if save_model:
        model.save_model('catboost_model_topK_full_filtered.cbm')
...
# Set the best iteration explicitly (since the best_iteration was obtained from cross-validation)
    ...
    if k > 0:
        ...
        if save_model:
            final_model.save_model('catboost_model_full_topk.cbm')
            return final_model
...
def CatBoost_Predicitons(model, X_val, y_val=None):
